...
# =============================================================================
# Polygon Options → Snowflake (Stream Loader) — dataset-triggered (latest pointer)
# -----------------------------------------------------------------------------
# Purpose:
#   Load *today's* ready data as soon as the daily ingest DAG updates the
#   "latest" POINTER file. No date math, no S3 prefix scans.
#
# Data contract:
#   • Latest pointer (mutable): raw/manifests/polygon_options_manifest_latest.txt
#       - File body: "POINTER=raw/manifests/options/YYYY-MM-DD/manifest.txt\n"
#   • Per-day manifest (immutable): raw/manifests/options/YYYY-MM-DD/manifest.txt
#       - Body is a newline-delimited list of S3 keys under:
#           raw/options/year=YYYY/month=MM/day=DD/underlying=<U>/contract=<C>.json.gz
#
# Snowflake:
#   • Destination table (landing): RAW.SOURCE_POLYGON_OPTIONS_RAW  (config via conn extras)
...
DEFAULT_ARGS = {
    ...
    "pool": "load_pool",  # separate pool from API calls
}


@dag(
    dag_id="polygon_options_load_stream",
    description="Dataset-triggered loader for daily Polygon options (reads latest POINTER → per-day manifest).",
    start_date=pendulum.datetime(2025, 1, 1, tz="UTC"),
    schedule=[S3_OPTIONS_MANIFEST_DATASET],   # ← data-aware trigger from the ingest_daily DAG
    ...
    tags=["load", "polygon", "options", "snowflake", "stream"],
)
def polygon_options_load_stream_dag():
    ...
    def resolve_snowflake_ctx() -> dict:
        """
        Pulls DB/schema/stage/table names from conn extras to keep code IaC-friendly.
    ...
    def read_per_day_manifest(pointer_or_inline: str) -> List[str]:
        """
        Returns a de-duplicated list of **stage-relative** keys limited to options/*.
        ...
        # Normalize to stage-relative: strip leading 'raw/' and filter to options/*
        ...
        if not out:
            raise AirflowSkipException("Resolved manifest has 0 eligible option files; skipping.")
        return out
