...
def polygon_options_load_dag():
    ...
    def create_snowflake_table():
        ...
        CREATE TABLE IF NOT EXISTS {FULLY_QUALIFIED_TABLE_NAME} (
            option_symbol TEXT, trade_date DATE, underlying_ticker TEXT, expiration_date DATE,
            strike_price NUMERIC(19, 4), option_type VARCHAR(4), open NUMERIC(19, 4),
            high NUMERIC(19, 4), low NUMERIC(19, 4), close NUMERIC(19, 4), volume BIGINT,
            vwap NUMERIC(19, 4), transactions BIGINT, inserted_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP()
        );
        """
    ...
    def load_data_to_snowflake(s3_keys: list[str]):
        ...
        FROM (
            SELECT
                ...
                $1:results[0]:o::FLOAT, $1:results[0]:h::FLOAT, $1:results[0]:l::FLOAT,
                $1:results[0]:c::FLOAT, $1:results[0]:v::BIGINT, $1:results[0]:vw::FLOAT,
