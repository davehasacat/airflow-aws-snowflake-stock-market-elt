...
# =============================================================================
# Polygon Stocks → Snowflake (Loader) — manual/backfill-friendly
# -----------------------------------------------------------------------------
# Loads raw JSON(.gz) produced by the stocks ingest DAGs from S3 (external stage)
# into a typed RAW table. Supports both:
#   • POINTER manifest:    single line "POINTER=raw/manifests/stocks/YYYY-MM-DD/manifest.txt"
#   • Flat manifest:       one 'raw/stocks/.../*.json[.gz]' key per line
#
...
#   • Stage-relative paths (strip 'raw/' → 'stocks/...') for COPY
#   • Emits SNOWFLAKE_STOCKS_RAW_DATASET for downstream dbt incremental models
#
# Usage patterns
...
from dags.utils.polygon_datasets import SNOWFLAKE_STOCKS_RAW_DATASET
...
@dag(
    ...
    description="Polygon stocks loader — reads POINTER or flat manifest and COPYs to Snowflake RAW.",
    ...
    params={
        ...
        "manifest_key": Param(
            default=os.getenv("STOCKS_MANUAL_MANIFEST_KEY", "raw/manifests/stocks/2025-10-20/manifest.txt"),
            type="string",
...
def polygon_stocks_load_dag():
    ...
    def read_manifest_keys(**context) -> List[str]:
        ...
        Returns an ordered list of stage-relative keys (stocks/*).
        """
        s3 = S3Hook()
