...
# =============================================================================
# Polygon Stocks → Snowflake (Load Stream)
# -----------------------------------------------------------------------------
# Stream loader subscribed to the *POINTER* manifest dataset produced by the
# daily stocks ingest DAG. It dereferences the pointer to an immutable per-day
# manifest, loads the listed JSON(.gz) files from S3 external stage into a typed
# RAW table, and emits a Snowflake dataset on success for downstream dbt jobs.
#
# Enterprise/modern behavior:
#   • Subscribes to S3_STOCKS_MANIFEST_DATASET (POINTER file)
#   • Pointer format: a single line →  "POINTER=raw/manifests/stocks/YYYY-MM-DD/manifest.txt"
...
#   • Emits SNOWFLAKE_STOCKS_RAW_DATASET for dbt incremental models
#   • Uses pools to control concurrency: load_pool + snowflake_pool
# =============================================================================
...
LATEST_POINTER_KEY = os.getenv(
    "STOCKS_MANIFEST_KEY", "raw/manifests/polygon_stocks_manifest_latest.txt"
)

# COPY tuning
BATCH_SIZE = int(os.getenv("SNOWFLAKE_COPY_BATCH_SIZE", "750"))       # FILES list size per COPY
...
@dag(
    ...
    description="Load stream for Polygon stocks: subscribe to POINTER manifest and COPY to Snowflake RAW.",
    start_date=pendulum.datetime(2025, 1, 1, tz="UTC"),
    schedule=[S3_STOCKS_MANIFEST_DATASET],   # fires when the POINTER file is updated
    catchup=False,
    tags=["load", "polygon", "snowflake", "stocks", "stream"],
...
def polygon_stocks_load_stream_dag():
    ...
    def read_pointer_and_manifest() -> List[str]:
        ...
        # 3) Normalize to stage-relative keys: strip leading 'raw/' and keep stocks/*
        ...
        print(f"Files eligible (stocks/*): {len(deduped)}")
        return deduped
