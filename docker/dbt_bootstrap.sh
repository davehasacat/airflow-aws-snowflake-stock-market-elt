#!/usr/bin/env bash
# Render dbt profiles.yml from AWS Secrets Manager and optionally validate with dbt parse.
set -euo pipefail
...
: "${DBT_BOOTSTRAP_VALIDATE:=1}"                    # 0 = skip dbt parse
...
resolve_dbt_bin() {
  ...
  if [ -x "$primary" ]; then echo "$primary"; return 0; fi
  if [ -x "/usr/local/bin/dbt" ]; then echo "/usr/local/bin/dbt"; return 0; fi
  if command -v dbt >/dev/null 2>&1; then command -v dbt; return 0; fi
  return 1
}

# ── Pre-flight
log "Starting bootstrap as user: $(whoami) (uid: $(id -u)) in $(pwd)"
require_bin aws
require_bin python
DBT_BIN_RESOLVED="$(resolve_dbt_bin)" || die "dbt executable not found at ${DBT_BIN} or /usr/local/bin/dbt, and not in PATH."
log "Using dbt binary: ${DBT_BIN_RESOLVED} ($( ${DBT_BIN_RESOLVED} --version 2>/dev/null || echo 'unknown version' ))"
log "Using aws binary at: $(command -v aws)"
mkdir -p "${DBT_DIR}"
...
for i in 1 2 3; do
  ...
  rc=$?
  set -e
  [ $rc -eq 0 ] && [ -n "${SECRET_JSON}" ] && break
  log "Retry $i/3: could not read secret (rc=$rc)."
  sleep $((i * 2))
done

if [ -z "${SECRET_JSON}" ]; then
  log "AWS error output:"; sed -e 's/^/[aws-cli] /' /tmp/dbt_bootstrap_aws_err.log || true
  die "Failed to retrieve secret after 3 attempts."
fi

# ── Render profiles.yml atomically
...
missing = []
if conn_type != "snowflake": missing.append("conn_type must be 'snowflake'")
for k, v in {
    "login": login,
...
print(next(iter(p.values()))["outputs"]["dev"]["private_key_path"])
PY
"${DBT_PROFILES_DIR%/}/profiles.yml")"
[ -f "${KEY_PATH}" ] || die "Private key file not found at ${KEY_PATH}. Check your './keys' volume mount and filename."
log "Found private key at ${KEY_PATH}"
