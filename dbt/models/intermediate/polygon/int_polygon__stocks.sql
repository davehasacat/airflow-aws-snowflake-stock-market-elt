...
with source as (
    select * from {{ ref('stg_polygon__stocks') }}
...
select
  ...
  avg(close_price) over (partition by ticker order by polygon_trade_date rows between 19 preceding and current row) as moving_avg_20d,
  avg(close_price) over (partition by ticker order by polygon_trade_date rows between 49 preceding and current row) as moving_avg_50d,
  avg(close_price) over (partition by ticker order by polygon_trade_date rows between 119 preceding and current row) as moving_avg_120d,
  (close_price - lag(close_price, 1) over (partition by ticker order by polygon_trade_date)) as price_change_1d,
