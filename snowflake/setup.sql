...
-- New functional schemas
CREATE SCHEMA IF NOT EXISTS STOCKS_ELT_DB.RAW
  COMMENT = 'Immutable source & dbt snapshots (landing/source models + snapshots)';
CREATE SCHEMA IF NOT EXISTS STOCKS_ELT_DB.PREP
  COMMENT = 'Staging + intermediate models (cleaned, typed, join-ready)';
...
CREATE USER IF NOT EXISTS airflow_stocks_user
  ...
  DEFAULT_NAMESPACE    = STOCKS_ELT_DB.PREP  -- UPDATED: dbt-friendly default
...
-- ------------------
-- Privileges (least privilege) — UPDATED for RAW/PREP/MARTS
-- ------------------
-- Warehouse access
...
-- Limit CREATE privileges to functional schemas (not PUBLIC)
-- RAW: loaders + snapshots may create/replace tables/views
...
-- (No CREATE privileges in PUBLIC to keep it clean/locked down)
...
GRANT USAGE
  ON ALL STAGES IN SCHEMA STOCKS_ELT_DB.RAW TO ROLE STOCKS_ELT_ROLE;
GRANT USAGE
  ON FUTURE STAGES IN SCHEMA STOCKS_ELT_DB.RAW TO ROLE STOCKS_ELT_ROLE;

GRANT USAGE
  ON ALL FILE FORMATS IN SCHEMA STOCKS_ELT_DB.RAW TO ROLE STOCKS_ELT_ROLE;
GRANT USAGE
